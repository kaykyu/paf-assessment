...
public class BookingsRepository {
	...
	public void newUser(User user) throws FailureException{
		if (template.update(SQL_INSERT_USER, user.email(), user.name()) <= 0)
			throw new FailureException("Failed to save user");
	}

	// TODO: Task 6
	// IMPORTANT: DO NOT MODIFY THE SIGNATURE OF THIS METHOD.
	// You may only add throw exceptions to this method
	public void newBookings(Bookings bookings) throws FailureException{

		if (template.update(SQL_INSERT_BOOKING,
				bookings.getBookingId(),
				bookings.getListingId(),
				bookings.getDuration(),
				bookings.getEmail()) <= 0)
			throw new FailureException("Failed to save booking");
	}
}
